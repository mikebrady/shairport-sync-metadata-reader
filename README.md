...
This sample program, `shairport-sync-metadata-reader`, reads the pipe of metadata optionally generated by Shairport Sync 2.4 and later.

[Shairport Sync](https://github.com/mikebrady/shairport-sync) is an AirPlay player with audio synchronization and multi-room capability.

To use `shairport-sync-metadata-reader`, read the pipe provided by Shairport Sync (default: `/tmp/shairport-sync-metadata`) via standard input, e.g:
 ...
 * `ssnc` for all metadata coming from Shairport Sync itself, such as start/end delimiters, etc.

* For `core` metadata, the second 4-character code is the 4-character metadata code that comes from the Apple Music app, etc. See, for example, https://code.google.com/p/ytrack/wiki/DMAP for information about the significance of the codes. The original data supplied by the source, if any, follows, and is encoded in Base64 format. The length of the data is also provided.
